...
<script setup>
    ...
    const signIn = () => {
        ...
        signInWithEmailAndPassword(auth, userData.email, userData.password)
            ...
            .catch((error) => {
                const displayErrMsg = error.message.includes('user-not-found')
                    ? 'User not found, try again or register first'
    ...
    const signup = () => {
        ...
        createUserWithEmailAndPassword(auth, userData.email, userData.password)
            ...
            .catch((error) => {
                errorMsg.value = error.message.includes('email-already-in-use')
    ...
    const emailRegex =
        /^[a-z0-9!#$%&'*+/=?^_`{|}~-]{1,64}(\.[a-z0-9!#$%&'*+/=?^_`{|}~-]{1,64})*@[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])*(\.[a-z]{2,})+$/i;
</script>
